...
matched %>%
  ...
  write_csv(here::here("data/derived_data/aireml_varcomp/fixed15/sanity_key.csv"),
            na = "")
...
matched %>%
  ungroup() %>%
  select(full_reg, cg_num, PC1, PC2, hair_score) %>%
  write_delim(here::here("data/derived_data/aireml_varcomp/fixed15/data.txt"),
              col_names = FALSE)
